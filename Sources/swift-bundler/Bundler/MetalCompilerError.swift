  ...
  var errorDescription: String? {
    switch self {
      case .failedToCreateTemporaryCompilationDirectory(let directory, _):
        return "Failed to create a temporary directory for shader compilation at '\(directory)'"
      case .failedToCompileShader(let file, let processError):
        return "Failed to compile shader source file '\(file)': \(processError.localizedDescription)"
      ...
      case .failedToDeleteShaderSource(let file, _):
        return "Failed to delete the shader source file at '\(file)' after compilation"
      case .failedToEnumerateShaders(let directory):
        return "Failed to enumerate shaders in directory '\(directory)'"
    }
  }
}
