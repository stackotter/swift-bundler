...
enum ResourceBundler {
  ...
  ///   - destination: The directory to copy the bundle to.
  ///   - minimumOSVersion: The minimum OS version that the app should run on. Used to created the bundle's `Info.plist`.
  ///   - platform: The platform that the app should run on.
  /// - Returns: If an error occurs, a failure is returned.
  static func fixAndCopyResourceBundle(
    ...
    let copyBundle = flatten(
      ...
      { createResourceBundleInfoPlist(in: destinationBundle, minimumOSVersion: minimumOSVersion, platform: platform) },
