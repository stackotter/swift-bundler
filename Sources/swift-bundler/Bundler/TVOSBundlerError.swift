...
/// An error returned by ``TVOSBundler``.
enum TVOSBundlerError: LocalizedError {
  ...
  case failedToGetMinimumTVOSVersion(manifest: URL)
  ...
  var errorDescription: String? {
    switch self {
      ...
      case .mustSpecifyBundleIdentifier:
        return "Bundle identifier must be specified for tvOS apps"
      case .failedToLoadManifest(let error):
        return error.localizedDescription
      case .failedToGetMinimumTVOSVersion(let manifest):
        return
          "To build for tvOS, please specify a tvOS deployment version in the platforms field of '\(manifest.relativePath)'"
    }
  }
}
