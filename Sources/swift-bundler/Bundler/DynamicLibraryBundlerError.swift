  ...
  var errorDescription: String? {
    switch self {
      case .failedToEnumerateDynamicLibraries(let directory, _):
        return "Failed to enumerate dynamic libraries in '\(directory)'"
      case .failedToCopyDynamicLibrary(let source, let destination, _):
        return "Failed to copy dynamic library from '\(source)' to '\(destination)'"
      ...
      case .failedToGetOutputPathRelativeToExecutable(let outputPath, let executable):
        return "Failed to get output path '\(outputPath)' relative to executable at '\(executable)'"
